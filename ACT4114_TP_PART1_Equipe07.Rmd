...
```{r HistPrime, message=FALSE, echo = FALSE, warning = FALSE, fig.cap="\\label{fig:PointPrem}Logarithme des réclamations en fonction de la prime totale"}
...
La variable «DrivAge» est la plus absente du jeu de données; elle est manquante dans `r round(sum(is.na(Veh$DrivAge)) / nrow(Veh) * 100, 2)`% des observations. Faison un diagramme boxplot pour voir s'il y a un lien entre la présence d'une observation manquante pour cette variable et le montant des réclamations.
...
```
Il semble peu probable que les données manquantes de «DrivAge» soient MCAR. Même que la figure précédente soulève la possibilité qu'on est en présence de donéées manquantes NMAR. Effectivement, les tests du khi-carré de Pearson rejettent l'hyopthèse nulle que le Patron de non-réponse de cette variable est MCAR. Il sera nécessaire d'imputer les données manquantes. Vu la possibilité que le patron de non-réponse soit NMAR, la méthode d'imputation stochastique multiple sera privilégié.
