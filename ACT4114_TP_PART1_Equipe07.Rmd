...
Selon la documentation d'*AUTOSEG*, cette variable correspond à la moyenne pondérée par exposition du montant de réclamation. Nous émettons l'hypothèse que cette variable représente la limite supérieure des réclamations possibles. Cette hypothèse est importante car elle pourrait guider notre manière de construire le modèle. On peut explorer cette hypothèse avec le graphique suivant:
...
On voit donc dans la Figure \ref{fig:PointSumInsAvg} que dans la très grande majorité des cas, le montant assuré est supérieur au montant réclamé. On peut croire que les points sous la courbe représentent des observations où le montant assuré a diminué (au renouvèlement par exemple). En effet, si le montant assuré a été plus élevé par le passé et a diminué subitement, suite à un changement de véhicule par exemple, il est possible que les réclamations normalisées dépassent le montant assuré normalisé. Nous croyons donc que notre hypothèse est raisonnable et nous considérons avoir bien compris le sens de cette variable.

La Figure \ref{fig:PointSumInsAvg} nous indique aussi qu'il y a une forte corrélation entre le montant assuré et le montant de réclamation. Elle aura donc un grand impact sur le modèle quant à sa con
ception et à nos règles de décisions pour le domaine des prédictions.
