...
Nous croyons que la variable sur le modèle de véhicule contient trop de niveaux, ce qui nuit à leur puissance prédictive et à leur interprétabilité. Nous avons donc choisi d'utiliser les expressions régulières pour regrouper les véhicules par marque. Nous faisons implicitement l'hypothèse que la gamme de prix au sein d'une même marque reste relativement la même. Cette simplification permet une meilleure interprétabilité. 
...
En analysant le lien avec les réclamations, on remarque qu'il n'y a absolument aucune réclamation pour le genre *Corporate*. Étant donné la très grande exposition qui y est associée, c'est évidemment absurde et on en déduit que l'assureur demande l'information sur le conducteur lorsqu'un accident survient et qu'une réclamation est nécessaire. Ainsi, il ajoute l'information rétrospectivement. Pour faire des prédictions, il sera possiblement nécessaire de régrouper le genre *Corporate* avec les clients pour lesquels l'information sur le genre est inconnue. 
...
On voit donc dans la figure \ref{fig:PointSumInsAvg} que dans la très grande majorité des cas, le montant assuré est supérieur au montant réclamé. On peut croire que les points sous la courbe représente des observations où le montant assuré a diminué (au renouvellement par exemple). En effet, si le montant assuré a été plus élevé par le passé et a diminué subitement, suite à un changement de véhicule par exemple, il est possible que les réclamations normalisées dépassent le montant assuré normalisée. Nous croyons donc que notre hypothèse est raisonnable et nous considérons avoir bien compris le sens de cette variable.

La figure \ref{fig:PointSumInsAvg} nous indique aussi qu'il y a une forte corrélation entre le montant assuré et le montant de réclamation. Elle aura donc un grand impact sur le modèle quant à sa conception et à nos règles de décisions pour le domaine des prédictions.
...
Commençons par examiner le motif des données manquantes en utilisant la fonction md.pattern() sur le jeu de données *Veh*.
\clearpage
```{r, echo=F, message=FALSE, echo=FALSE, warning=FALSE, results='hide', fig.keep='all', fig.cap="\\label{fig:MissingDataVis}Visualisation des données manquantes"}
md.pattern(Veh, rotate.names = T)
```
Nous constatons que lorsque le modèle de véhicule est manquant pour une observation, les valeurs correspondantes pour les variables VehGroup et VehManuf sont également manquantes. Pour déterminer si les variables *VehModel* et *VehGroup* fournissent des informations utiles pour notre analyse, nous en faisons une courte analyse: 
```{r, echo=F}
...
Nous avons remarqué que la variable VehManuf est plus informative que les variables VehModel et VehGroup. En effet, la marque d'un véhicule peut avoir un impact significatif sur le risque associé à une observation, alors que la différence de risque entre différents modèles d'une même marque peut être relativement faible. Ainsi, en utilisant uniquement la variable VehManuf, nous pouvons réduire la complexité de l'analyse tout en conservant les informations les plus pertinentes pour notre objectif d'analyse.

Par conséquent, nous avons décidé de retirer les variables VehModel et VehGroup du jeu de données pour simplifier l'imputation des données manquantes et l'analyse ultérieure. De plus, cette action permet de réduire la dimensionnalité du jeu de données, ce qui peut conduire à des modèles plus simples et plus facilement interprétables.
```{r, echo=F}
temp_data <- Veh[, !names(Veh) %in% c("VehModel", "VehGroup")]
```

Il n'est pas possible d'utiliser les cas complets pour analyser les données manquantes dans notre jeu de données car les données manquantes ne sont pas manquantes complètement au hasard (MCAR). En effet, il existe une certaine structure dans les données manquantes, ce qui peut introduire un biais important dans l'analyse si les observations manquantes sont simplement ignorées. Par conséquent, nous avons choisi d'utiliser le package **mice** pour effectuer une imputation par forêt aléatoire, qui permet de prendre en compte la structure des données manquantes et de minimiser les biais potentiels introduits par l'imputation.
```{r, echo=F}
...
Après avoir effectué l'imputation des données manquantes à l'aide du package mice, toutes les valeurs manquantes ont été remplacées dans le jeu de données. Par conséquent, le jeu de données est désormais complet et prêt à être utilisé pour l'analyse.
